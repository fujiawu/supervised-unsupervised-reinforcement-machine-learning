...
function k_nearest_neighbor_mushroom
...
% create a knn predictor
knn = fitcknn(X_training(1:trainig_size(j),:),Y_training(1:trainig_size(j)), ...
                  'Distance','euclidean','NumNeighbors',5);
              
%testing
Y_training_prediction = predict(knn,X_training);
Y_test_prediction = predict(knn,X_test);
...
%set(le, 'box', 'off');
text(mean(trainig_size)/2, 0.3, 0, 'k-nearest neighbors - Mushroom Data','Fontsize', 16);
save;
