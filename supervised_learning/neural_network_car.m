...
data = String2Matrix(rawdata);
inputs = data(:,1:6)';
targets = String2BinaryMaxtrix(rawdata{7});
...
%set(le, 'box', 'off');
text(mean(trainig_size)/2, 0.3, 0, 'Neural Network - Car Data','Fontsize', 16);
save;
...
function matrix = String2Matrix(stringMatrix)
    ...
    for i = 1:length(stringMatrix)  
      matrix = [matrix, String2Num(stringMatrix{i},i)];
    end
return


%% convert string array to normalized numeric array
function numArray = String2Num(stringArray, col)
